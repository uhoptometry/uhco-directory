component output="false" singleton {
    ...
    public any function init() {
        variables.OrganizationsDAO = createObject("component", "dao.organizations_DAO").init();
        return this;
    }
    ...
    public struct function createOrg( required string orgName, string orgType="", any parentOrgID="", numeric additionalRoles=0, string orgDescription="", numeric display=0 ) {
        var newID = variables.OrganizationsDAO.createOrg( orgName, orgType, parentOrgID, arguments.additionalRoles, arguments.orgDescription, arguments.display );
        return { success=true, message="Organization created.", orgID=newID };
    }

    public struct function updateOrg( required numeric orgID, required string orgName, string orgType="", any parentOrgID="", numeric additionalRoles=0, string orgDescription="", numeric display=0 ) {
        variables.OrganizationsDAO.updateOrg( orgID, orgName, orgType, parentOrgID, arguments.additionalRoles, arguments.orgDescription, arguments.display );
