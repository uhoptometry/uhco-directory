...
<!--- Pre-filter: only users where Active = 0 --->
<cfset inactiveUsers = []>
<cfloop from="1" to="#arrayLen(allUsers)#" index="i">
    <cfif val(allUsers[i].ACTIVE) EQ 0>
        <cfset arrayAppend(inactiveUsers, allUsers[i])>
    </cfif>
</cfloop>
<cfset allUsers = inactiveUsers>
...
<div class='d-flex justify-content-between mb-4'>
    <h1>Inactive Users <span class='badge bg-secondary fs-6'>#totalRecords#</span></h1>
