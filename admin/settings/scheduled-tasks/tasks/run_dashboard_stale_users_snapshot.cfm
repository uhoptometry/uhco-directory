...
<!---
    run_dashboard_stale_users_snapshot.cfm
    Computes stale-user totals for dashboard cards and stores the latest
...
<cftry>
    <cfset pageResult = usersDAO.getStaleUsersForDashboardPage(pageSize=1, pageNumber=1, staleMonths=staleMonths)>
    ...
    <cfset appConfigService.setValue("scheduled_tasks.dashboard_stale_users.last_run", serializeJSON(snapshot))>
    <cfset success = true>
<cfcatch type="any">
...
<cfif success>
    <cflocation url="#request.webRoot#/admin/settings/scheduled-tasks/?msg=ran&taskKey=UHCO_DashboardStaleUsers&total=#urlEncodedFormat(total)#" addtoken="false">
<cfelse>
    <cflocation url="#request.webRoot#/admin/settings/scheduled-tasks/?msg=error&taskKey=UHCO_DashboardStaleUsers&err=#urlEncodedFormat(errorMsg)#" addtoken="false">
</cfif>
